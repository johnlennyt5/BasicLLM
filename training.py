...
parser.add_argument('-batch_size', type=int, required=True, help='please provde a batch_size')
...
# memory map for using a small snippets of text from a single file of any size
def get_random_chunk(split):
    filename= "openwebtext/output_train.txt" if split == 'train' else "openwebtext/output_val.txt"
    with open(filename, 'rb') as f:
        with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
            #Determine the file size and a rnadom position to start reading
            file_size = len(mm)
            ...
            #Decode the block to a string, ignorign any valid byte sequences
